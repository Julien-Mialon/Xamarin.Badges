...
internal class SamsungHomeBadger : DefaultBadger
{
	...
	{
		if (Build.VERSION.SdkInt >= BuildVersionCodes.LollipopMr1 && IsSupported(context)) //new samsung >= 5.1 use default badger system
		{
			//use default badger
		...
		try
		{
			cursor = resolver.Query(contentUri, ContentProjection, "package=?", [component.PackageName], null);
			if (cursor == null)
			{
				return false;
			}
			...
			while (cursor.MoveToNext())
			{
				...
				resolver.Update(contentUri, contentValues, "_id=?", [id.ToString()]);
	...
	public override string[] SupportedLaunchers => ["com.sec.android.app.launcher", "com.sec.android.app.twlauncher"];
}